        ...
        return function (ws) {
            ...
            ws.on('close', function () {
                ...
                if (nodeName) {
                    ...
                    this.kevs.parse(this.tpl(this.group.getDictionary().getString('onDisconnect', ''), nodeName), modelToApply, function (err, model) {
                        if (err) {
                            this.log.error(this.toString(), 'Unable to parse onDisconnect KevScript. No changes made after the disconnection of '+nodeName);
                        } else {
                        ...
                        case Protocol.REGISTER_TYPE:
                            if (!this.name2Ws[parsedMsg.getNodeName()]) {
                                ...
                                if (this.group.isMaster()) {
                                    ...
                                    // add onConnect logic
                                    this.kevs.parse(this.tpl(this.group.getDictionary().getString('onConnect', ''), parsedMsg.getNodeName()), modelToApply, function (err, model) {
                                        if (err) {
