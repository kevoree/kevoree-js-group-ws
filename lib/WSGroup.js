...
var WSGroup = AbstractGroup.extend({
  ...
  onPortUpdate: function (newVal, oldVal) {
    this.updatePort = true;
  },
  onMasterUpdate: function (newVal, oldVal) {
  ...
  createServer: function(done) {
    try {
      var that = this;
      var port = this.dictionary.getNumber('port', WSGroup.prototype.dic_port.defaultValue);
